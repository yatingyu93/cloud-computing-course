...
def isvalid_city(city):
    valid_cities = ['new york', 'brooklyn', 'manhattan', 'queens', 'los angeles', 'chicago', 'houston', 'philadelphia',
                    'san diego', 'dallas', 'san jose', 'austin', 'jacksonville', 'san francisco', 'indianapolis',
                    'columbus', 'fort worth', 'charlotte', 'detroit', 'el paso', 'seattle', 'denver', 'washington dc',
                    'memphis', 'boston', 'nashville', 'baltimore', 'portland', 'phoenix', 'san antonio']
    return city.lower() in valid_cities


def validate_dining_info(cuisine_type, date, dining_time, num_people, email, city):
    cuisine_types = ['chinese', 'american', 'italian', 'indian', 'greek', 'french', 'japanese']
    if cuisine_type is not None and cuisine_type.lower() not in cuisine_types:
        return build_validation_result(False,
                                       'Cuisine',
                                       'We do not have {}, would you like a different type of cuisine? (French, Italian, etc...)'
                                       .format(cuisine_type))
    istoday = False
    if date is not None:
